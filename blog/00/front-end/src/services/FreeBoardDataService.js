...
class FreeBoardDataService {
  // freeBoards 전체 목록 조회 메소드
  // !전체 게시판 조회
  // *axios -> spring 데이터 요청 -> spring -> db -> 결과 반환
  // Todo : getAll => getAll(params)
  getAll (params) {
    return http.get('/free-boards', { params })
  }

  // id로 조회하는 메소드
  // !게시판 id값으로 조회
  get (id) {
    return http.get(`/free-boards/${ id }`)
  }

  // freeBoard 데이터 추가 메소드
  // !게시판 데이터 생성
  create (data) {
    return http.post('/free-boards', data)
  }

  // freeBoard 데이터 수정 메소드
  // !게시판 데이터 수정
  update (id, data) {
    return http.put(`/free-boards/${ id }`, data)
  }

  // freeBoard 데이터 삭제 메소드
  // !게시판 데이터 삭제
  delete (id) {
    return http.put(`/free-boards/deletion/${ id }`)
  }

  // freeBoard 모든 데이터 삭제 메소드
  // !모든 게시판 데이터 삭제
  deleteAll () {
    return http.put(`/free-boards`)
  }

  // freeBoard 데이터 중에 title을 기준으로 검색하는 메소드
  // * 전통적인 웹 개발 방식을 spring에서도 사용중이라서 url 주소도 똑같은 방식으로 들어가야함
  // * URL : http://loccalhost:8080/board?name=forbob
  findByTitle (title) {
    return http.get(`/free-boards?title=${ title }`)
  }
}
