<template>
  <div class = "container mt-3">
      ...
      <div class = "input-group mb-3">
        ...
        <div class = "input-group-append">
          <button class = "btn btn-outline-dark"
                  type = "button"
                  @click = "page =1; retrieveFAQBoards();"
          >
            Search Title
          </button>
        </div>
    ...
    <table class = "table">
      ...
      <!--#조회 데이터 생 부분-->
      <!--Todo: :class, @click 추가-->
      <tbody v-for = "(faqBoard, index) in faqBoards"
             :key = "index"
             :class = "{ active: index == currentIndex}"
             @click = "setActiveFAQBoard(faqBoard,index)"
      >
      <tr>
        <td>{{ faqBoard.id}}</td>
        <td>{{ faqBoard.title }}</td>
        <td>{{ faqBoard.content }}</td>
        <!--* http://localhost8080/faqBoards/faqBoard.id 형태의 주소로 이동-->
        <!--* :href -> v-bind:href, faqBoard.id 값을 가져오기 위해서는 <script>의 data()와 바인딩할 필요가 있음-->
        <td><a :href = "'/faq-boards/' + faqBoard.id"
  ...
  methods: {
    ...
    retrieveFAQBoards () {
      ...
      FAQBoardDataService.getAll(params)
          ...
          // *data : back-end(스프링)에서 보내준 faqBoard 객체가 담김
          ...
          .then(response => {
            ...
            // 임시변수 faqBoards, totalItems( 서버의 결과 데이터가 들어옴 )
            const {
                    faqBoards,
                    totalItems,
                  } = response.data
            this.faqBoards = faqBoards // 객체
    ...
    // 새로고침 메소드
    refreshList () {
      this.currentFAQBoard = null
      this.currentIndex = -1
      this.retrieveFAQBoards()
    },
    // vue의 data 변수에 값을 저장하는 메소드(클릭했을 때 현재값 저장)
    setActiveFAQBoard (faqBoard, index) {
      this.currentFAQBoard = faqBoard
