...
public class BoardController {
    ...
    public String BoardList(Model model,
        ...
        // *페이지의 시작 값, 현재 페이지에서 -1을 빼줘서 원복시킨 후 계산
        int startPageNum = (int)(Math.ceil((nowPage-1)*1.0)/pageable.getPageSize())*pageable.getPageSize();
        int startPage = startPageNum + 1;

        // *페이지의 마지막 값, 시작 페이지에서 +9개를 더해 총 10개의 블록을 보여주고, 전체 페이지 수를 넘어가면 마지막 페이지 값을 전체 페이지 값으로 반환
        int endPage = Math.min(startPage +9, boards.getTotalPages());
